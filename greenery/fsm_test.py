...
from greenery.fsm import anything_else, epsilon, fsm, null
...
def test_reduce():
    ...
    asdf = fsm(
        ...
        initial=0,
        finals={1},
        map={0: {None: 2}, 1: {None: 2}, 2: {None: 2}},
    )
    asdf = asdf.reduce()
...
def test_invalid_fsms():
    ...
    try:
        fsm(alphabet={"a"}, states={1}, initial=1, finals=set(), map={1: {"a": 2}})
        assert False
    except AssertionError:
        assert False
    except Exception:
...
def test_empty(a, b):
    ...
    assert fsm(
        alphabet={}, states={0, 1}, initial=0, finals={1}, map={0: {}, 1: {}},
    ).empty()

    assert not fsm(alphabet={}, states={0}, initial=0, finals={0}, map={0: {}},).empty()
...
def test_alphabet_unions():
    ...
    a = fsm(alphabet={"a"}, states={0, 1}, initial=0, finals={1}, map={0: {"a": 1}})

    b = fsm(alphabet={"b"}, states={0, 1}, initial=0, finals={1}, map={0: {"b": 1}})
...
def test_new_set_methods(a, b):
    ...
    for string in four:
        assert string == ["a", "a"]
        break
    assert list(four) == [["a", "a"], ["a", "b"], ["b", "a"], ["b", "b"]]
