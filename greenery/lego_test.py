...
def test_mult_equality():
    ...
    assert mult(charclass("a"), one) != mult(
        charclass("a"), multiplier(bound(1), bound(2))
    )
...
def test_conc_equality():
    ...
    assert conc(mult(charclass("a"), one)) != conc(
...
def test_pattern_equality():
    assert pattern(
        conc(mult(charclass("a"), one)), conc(mult(charclass("b"), one)),
    ) == pattern(conc(mult(charclass("b"), one)), conc(mult(charclass("a"), one)),)
    assert pattern(
        conc(mult(charclass("a"), one)), conc(mult(charclass("a"), one)),
    ) == pattern(conc(mult(charclass("a"), one)),)
...
def test_w_d_s():
    ...
    assert charclass.parse(r"\w") == charclass.parse(
        "[0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz]"
    )
    assert charclass.parse(r"[\w~]") == charclass.parse(
        "[0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~]"
    )
    assert charclass.parse(r"[\da]") == charclass.parse("[0123456789a]")
    assert charclass.parse(r"[\s]") == charclass.parse("[\t\n\r\f\v ]")
...
def test_mult_parsing():
    ...
    assert mult.parse("[a-g0-8$%\\^]+") == mult(charclass("abcdefg012345678$%^"), plus)
    assert mult.match("abcde[^fg]*", 5) == (mult(~charclass("fg"), star), 11)
...
def test_conc_parsing():
    ...
    assert conc.parse("[bc]*[ab]*") == conc(
        mult(charclass("bc"), star), mult(charclass("ab"), star),
    )
...
def test_pattern_str():
    assert (
        str(pattern(conc(mult(charclass("a"), one)), conc(mult(charclass("b"), one)),))
        == "a|b"
    )
    assert (
        str(pattern(conc(mult(charclass("a"), one)), conc(mult(charclass("a"), one)),))
        == "a"
    )
    assert (
        str(
...
def test_wildcards_in_charclasses():
    # Allow "\w", "\d" and "\s" in charclasses
    assert parse(r"[\w~]*").matches("a0~")
    assert parse(r"[\da]*").matches("0129a")
    assert parse(r"[\s]+").matches(" \t \t ")
...
def test_forin():
    assert list(parse("abc|def(ghi|jkl)")) == ["abc", "defghi", "defjkl"]
...
@pytest.mark.xfail(strict=True, reason="previously concealed by test below")
...
def test_base_N():
    ...
    assert base <= 10
    known_map = {
        "initial": {"0": "zero", **{str(j): j % N for j in range(1, base)}},
        "zero": {str(j): None for j in range(base)},
        None: {str(j): None for j in range(base)},
    }
    i_map = {i: {str(j): (i * base + j) % N for j in range(base)} for i in range(N)}
    divN = from_fsm(
        fsm.fsm(
            alphabet={str(i) for i in range(base)},
            states=set(i_map) | set(known_map),
            initial="initial",
            finals={"zero", 0},
            map=dict((*known_map.items(), *i_map.items())),  # noqa: C406
        )
    )
    gen = divN.strings()
    a = next(gen)
...
def test_isinstance_bug():
    ...
    (
        parse("").everythingbut() + parse("aa") + parse("").everythingbut()
    ).everythingbut()
...
def test_multiplier_subtraction():
    ...
    assert multiplier.parse("{3,4}").common(
        multiplier.parse("{2,5}")
    ) == multiplier.parse("{2,3}")
    assert multiplier.parse("{3,4}") - multiplier.parse("{2,3}") == one
    assert multiplier.parse("{2,5}") - multiplier.parse("{2,3}") == multiplier.parse(
        "{0,2}"
    )

    # a{2,}, a{1,5} -> a{1,5} (with a{1,}, a{0,0} left over)
    assert multiplier.parse("{2,}").common(
        multiplier.parse("{1,5}")
    ) == multiplier.parse("{1,5}")
    ...
    assert multiplier.parse("{3,}").common(
        multiplier.parse("{2,}")
    ) == multiplier.parse("{2,}")
...
def test_conc_common():
    assert conc.parse("AAZY").common(conc.parse("BBZY"), suffix=True) == conc.parse(
        "ZY"
    )
...
def test_pattern_dock():
    ...
    assert pattern.parse("xyza|abca|a").dock(conc.parse("a")) == pattern.parse(
        "xyz|abc|"
    )
    assert pattern.parse("f{2,3}c|fc").dock(conc.parse("fc")) == pattern.parse(
        "f{1,2}|"
    )
...
def test_pattern_beheading():
    ...
    assert pattern.parse("cf{1,2}|cf").behead(conc.parse("c")) == pattern.parse(
        "f{1,2}|f"
    )
    ...
    assert pattern.parse("Z+A|ZB|ZZC").behead(conc.parse("Z")) == pattern.parse(
        "Z*A|B|ZC"
    )
...
def test_pattern_multiplication():
    assert pattern.parse("ab?|ba?") * multiplier.parse("{2,3}") == mult.parse(
        "(ab?|ba?){2,3}"
    )
    # TODO
...
def test_empty_mult_suppression():
    assert conc.parse(r"[]0\d").reduce() == charclass.parse("[]")
    assert conc(
...
def test_empty_conc_suppression():
    assert pattern.parse(r"[]0\d").reduce() == charclass.parse("[]")
    assert pattern(
        conc(
            mult(pattern(), one),  # this mult can never actually match anything
            mult(charclass("0"), one),
...
def test_mult_squoosh():
    ...
    assert conc.parse("[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]").reduce() == mult.parse(
        "[0-9A-Fa-f]{3}"
    )
    assert conc.parse("[$%\\^]?[$%\\^]").reduce() == mult.parse("[$%\\^]{1,2}")
    assert conc.parse(
        r"(|(|(|(|(|(|[$%\^])[$%\^])[$%\^])[$%\^])[$%\^])[$%\^])[$%\^][$%\^]"
    ).reduce() == mult.parse(r"[$%\^]{2,8}")


def test_bad_reduction_bug():
    # DEFECT: "0{2}|1{2}" was erroneously reduced() to "[01]{2}"
    assert parse("0{2}|1{2}").reduce() != parse("[01]{2}")
    assert parse("0|[1-9]|ab").reduce() == pattern.parse(r"\d|ab")
    assert parse("0|[1-9]|a{5,7}").reduce() == pattern.parse(r"\d|a{5,7}")
    assert parse("0|(0|[1-9]|a{5,7})").reduce() == pattern.parse(r"0|(\d|a{5,7})")
