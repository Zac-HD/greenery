...
class charclass(lego):
    ...
    def to_fsm(self, alphabet=None):
        ...
        if self.negated:
            map = {0: {symbol: 1 for symbol in alphabet - self.chars}}
        # If normal, make a singular FSM accepting only these characters
        else:
            map = {0: {symbol: 1 for symbol in self.chars}}
    ...
    def match(cls, string, i=0):
        ...
        def matchClassInterior1(string, i):
            ...
            try:
                ...
                chars = "".join([chr(i) for i in range(firstIndex, lastIndex + 1)])
                return chars, k
            except nomatch:
                pass
...
class multiplier:
    ...
    def __and__(self, other):
        ...
        if not self.canintersect(other):
            raise Exception(
                "Can't compute intersection of " + repr(self) + " and " + repr(other)
            )
    ...
    def __or__(self, other):
        ...
        if not self.canunion(other):
            raise Exception(
                "Can't compute the union of " + repr(self) + " and " + repr(other)
            )
...
class mult(lego):
    ...
    def intersection(self, other):
        ...
        try:
            if self.multiplicand == other.multiplicand and self.canintersect(other):
                return mult(self.multiplicand, self.multiplier & other.multiplier)
        except AttributeError:
    ...
    def reduce(self):
        ...
        try:
            if (
                emptystring in self.multiplicand.concs
                and self.multiplier.canmultiplyby(qm)
            ):
                return mult(
                    pattern(*self.multiplicand.concs.difference({emptystring})),
                    self.multiplier * qm,
                )
        except AttributeError:
        ...
        # zero times
        if self.multiplicand.empty() and self.multiplier.min == bound(0):
            return emptystring
...
class conc(lego):
    ...
    def reduce(self):
        ...
        reduced = [pattern(x) if hasattr(x, "mults") else x for x in reduced]
        reduced = [
    ...
    def dock(self, other):
        ...
        for i in reversed(range(len(other.mults))):  # [2, 1, 0]
            ...
            # then tries to subtract the B too, which isn't possible
                ...
                if i != 0:
                    raise Exception(
                        "Can't subtract " + repr(other) + " from " + repr(self)
                    )
...
class pattern(lego):
    ...
    def reduce(self):
        ...
        reduced = [conc(x) if hasattr(x, "multiplicand") else x for x in reduced]
